...
type SetChangeListener interface {
	SetChanged(tx *bbolt.Tx, rowId []byte, old []FieldTypeAndValue, new []FieldTypeAndValue, holder errorz.ErrorHolder)
}
...
func (index *setIndex) processAfterUpdate(ctx *IndexingContext) {
	if !ctx.errHolder.HasError() {
		...
		for _, listener := range index.listeners {
			listener.SetChanged(ctx.tx, ctx.rowId, oldValues, newValues, ctx.errHolder)
		}
	}
}
